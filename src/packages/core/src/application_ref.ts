...
export class PlatformRef {
  ...
  bootstrapModuleFactory<M>(
    ...
    const ngZoneEventCoalescing =
      (options && options.ngZoneEventCoalescing) || false;
    const ngZoneRunCoalescing =
      (options && options.ngZoneRunCoalescing) || false;
    ...
    return ngZone.run(() => {
      ...
      const exceptionHandler: ErrorHandler | null = moduleRef.injector.get(
        ErrorHandler,
        null
      );
      if (!exceptionHandler) {
      ...
      return _callAndReportToErrorHandler(exceptionHandler, ngZone!, () => {
        ...
        return initStatus.donePromise.then(() => {
          if (ivyEnabled) {
            ...
            const localeId = moduleRef.injector.get(
              LOCALE_ID,
              DEFAULT_LOCALE_ID
            );
...
function getNgZone(
  ...
  extra?: { ngZoneEventCoalescing: boolean; ngZoneRunCoalescing: boolean }
): NgZone {
  let ngZone: NgZone;
...
export class ApplicationRef {
  ...
  constructor(
    ...
    const isCurrentlyStable = new Observable<boolean>(
    ...
    (this as { isStable: Observable<boolean> }).isStable = merge(
      isCurrentlyStable,
      isStable.pipe(share())
    );
  }
  ...
  bootstrap<C>(
    ...
    const testabilityRegistry =
      testability && compRef.injector.get(TestabilityRegistry);
